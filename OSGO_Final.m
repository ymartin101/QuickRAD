...
% EEE4022S (2018)
% OSGO_Final.m: Final implementation of OSGO-CFAR to measured datasets
...
%% Plot PDF
pdf_x = abs(Cdata(:)).^2;	% create column vector from clutter data
Ns = length(pdf_x);
delta = 0.05;               % dimension of a range bin when computing the histogram
yapl = 0:delta:max(pdf_x);  % number of range bins

% Compute and plot histogram
histo = hist(pdf_x,yapl)./(Ns*delta);
figure; 
semilogy(yapl,histo,'.'); 
grid on;
title('Clutter PDF After Square Law Detector');
xlabel('Return Strength');
ylabel('Logarithmic Probability Density');
sum(delta*histo)  % area under histogram should = 1
