...
% EEE4022S (2018)
% PD_Multiple.m: Determine PFA and PD for a varying parameter in a "multiple" dataset using OSGO-CFAR

%% FUNCTION: Used to determine PFA and PD for a varying parameter
function PD_mean = PD_Multiple(N)  % change input parameter accordingly
...
%% Insert second target (away from radar)
    ...
    for RangeBin = 1:NumOfRangeBins
        ...
        kc1 = t1*kc/NumOfPRIs;      % prevent zero indices
        if t1 == 0
            kc1 = 1;
        end
